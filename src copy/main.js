...
function initVBOs(currScheme){
    ...
    plane.init();
    var cube = new VBO_genetic(currScheme[0], currScheme[1], cube_vertices, cube_colors, cube_normals, cube_indices, currScheme[2], 7);
    cube.init();
    ...
    sphere.init();
    vboArray = [grid, plane, cube, sphere];
}
...
function main() {
    ...
    // gl.clearColor(0.0, 0.0, 0.0, 0.0); //with background
    gl.enable(gl.DEPTH_TEST);
    gl.enable(gl.BLEND);// Enable alpha blending
    gl.blendFunc(gl.SRC_ALPHA, gl.ONE_MINUS_SRC_ALPHA); // Set blending function conflict with shadow...?
    g_modelMatrix = new Matrix4(); 

    shadingScheme = {  // [vert, frag shader, light Spec, material code]
        0:[draggableBlinnPhongVert, draggableBlinnPhongFrag, 1], 
    };
